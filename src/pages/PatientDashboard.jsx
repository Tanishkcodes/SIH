...
import { DASHBOARD_I18N } from '../constants/dashboardI18n';
...
export default function PatientDashboard() {
  ...
  // Dynamic Database Doctors & Hospitals & Communities
  const [dbDoctorsList, setDbDoctorsList] = useState([]);
  const [dbHospitalsList, setDbHospitalsList] = useState([]);
  const [dbCommunitiesList, setDbCommunitiesList] = useState([]);
  ...
  // Selected Community ID for Voice Navigation
  const [selectedCommunityId, setSelectedCommunityId] = useState(null);

  const handleOpenCommunity = (commandOrTarget) => {
    ...
    setBookingFlowView('main');
    const target = typeof commandOrTarget === 'string'
      ? commandOrTarget
      : (commandOrTarget?.target || commandOrTarget?.value || commandOrTarget?.raw || '');
    if (target) {
      setSelectedCommunityId(target);
      setTimeout(() => {
        window.dispatchEvent(new CustomEvent('swasthya_open_community', { detail: { query: target } }));
      }, 50);
    }
    return true;
  };

  // Fetch dynamic doctors & hospitals & communities from Supabase database
  useEffect(() => {
    let active = true;
    async function loadCatalog() {
      try {
        const [docsRes, hospsRes, commsRes] = await Promise.all([
          db.doctors.getAll(),
          db.hospitals.getAll(),
        ...
        if (active) {
          ...
          if (commsRes.data && commsRes.data.length > 0) {
            setDbCommunitiesList(commsRes.data);
          }
        }
      } catch (err) {
        console.warn('Unable to load live hospital/doctor/community catalog:', err);
      }
    }
    loadCatalog();
    return () => { active = false; };
  ...
  const handleOpenDoctorProfile = (doctor) => {
    if (!doctor) return;
    const hosp = bookingHospital || doctor.hospitalObj || hospitals.find(h => h.id === doctor.hospitalId || h.id === doctor.hospital_id) || hospitals[0];
    if (hosp) setBookingHospital(hosp);
    if (doctor.isAyush || hosp?.isAyush) setDoctorCareSystem('ayurveda');
    const fullProfile = getDoctorFullProfile(doctor, hosp || bookingHospital);
    setSelectedDoctorObj(fullProfile);
    setShowBookingModal(false);
    setShowAllHospitalsModal(false);
    setShowAbhaModal(false);
  ...
  useEffect(() => {
    ...
    const { selectHospital: openHospital, selectDoctor, openDoctorProfile } = createPatientSelectionActions({
      hospitals, doctors: allDoctorsAcrossHospitals, selectedHospital: bookingHospital, selectedDoctor: selectedDoctorObj, hospitalAliases: aliases, openTab,
    ...
    registerPage('patientDashboard', {
      ...
      select_doctor: (command) => {
        ...
        if (isProfile) return openDoctorProfile(command);
        return selectDoctor(command);
      },
      view_doctor_profile: openDoctorProfile,
      ...
      viewCommunities: (command) => {
        const query = command?.target || command?.value || command?.raw || '';
        const hasSpecific = query && !/^(communities|community|samudaay|samaj|viewCommunities)$/i.test(String(query).trim());
        if (hasSpecific) {
          handleOpenCommunity(query);
        } else {
          openTab('communities');
        }
        return true;
      },
      select_community: (command) => {
        handleOpenCommunity(command);
        return true;
      },
      open_community: (command) => {
        handleOpenCommunity(command);
        return true;
      },
      ...
      view_doctor_profile: [`Open doctor profile, physician biography, qualifications and reviews. Put its exact id in target and name in value. Available doctors: ${JSON.stringify(doctorContext)}`],
      doctor_profile: [`Open doctor profile, physician biography, qualifications and reviews. Available doctors: ${JSON.stringify(doctorContext)}`],
      ...
      viewCommunities: [`View patient communities tab or open a specific community by name or topic. Available communities: ${JSON.stringify(communityContext)}`],
      select_community: [`Open a specific health community by name, condition, topic, or one-based number. Put its exact id in target and title in value. Available communities: ${JSON.stringify(communityContext)}`],
    ...
    showAbhaModal, selectedAppointment, selectedDoc, filteredHospitals, hospitals, dbDoctorsList, dbCommunitiesList, allDoctorsAcrossHospitals, currentLang,
    selectedCommunityId
  ]);
                          ...
                          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: '0.9rem' }}>
                            {visibleSlots.map((slot) => {
                              ...
                              return (
                                ...
                                >
                                  <div className="notranslate" translate="no" style={{ fontSize: '0.925rem', fontWeight: '800', letterSpacing: '-0.2px' }}>
                                    {slot.label}
                                  </div>
                                ...
                                <div style={{ flex: 1, minWidth: 0 }}>
                                  ...
                                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px', whiteSpace: 'nowrap', flexWrap: 'nowrap' }}>
                                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', fontSize: '0.74rem', color: '#64748b', whiteSpace: 'nowrap', flexShrink: 0 }}>
                                      <Clock size={12} />
                                      <span className="notranslate" translate="no">{apt.time}</span>
                                    </span>
                                    {tokenPill && (
                                      <>
                      ...
                      <div style={{
                        ...
                        {allSlots.map(slot => {
                          ...
                          return (
                            ...
                            >
                              <div className="notranslate" translate="no" style={{ fontSize: '0.84rem' }}>{slot.label || slotTime}</div>
                              <div style={{
