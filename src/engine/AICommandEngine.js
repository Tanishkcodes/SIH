...
class AICommandEngine {
  ...
  _multilingualFallbackIntent(transcript, availableCommands = {}, globalCommands = {}, ctx = {}) {
    ...
    // 2b. Doctor Profile (checked before booking appointments)
    if (
      /\b(?:doctor profile|dr profile|doctor details|doctor bio|about doctor|doctor info|profile of doctor|profile of dr|doctor ki profile|doctor ka profile|doctor jankari|doctor parichay)\b/i.test(raw) ||
      (/\b(?:doctor|dr|chikitsak|vaidya|maruthuvar|vaidyudu)\b/i.test(raw) && /\b(?:profile|details|info|bio|about|parichay|jankari|vivaran)\b/i.test(raw))
    ) {
    ...
    // 8. Specific Community & Support Groups (dynamically extracted)
    if (
      /\b(?:community|group|circle|forum|mandal|samuh|charcha)\b/i.test(raw)
    ) {
      const extractedTopic = raw
        .replace(/\b(?:open|join|view|go to|chalo|kholo|show|dikhao|enter|community|group|circle|forum|mandal|samuh|charcha|patient|the|a|an|in|me|mein|tab|particular)\b/gi, ' ')
        .replace(/\s+/g, ' ')
        .trim();
      if (extractedTopic.length >= 2) {
        return { intent: 'select_community', value: extractedTopic, topic: extractedTopic, confidence: 0.95, message: `Opening ${extractedTopic} community.` };
      }
    }

    // 8b. Community & Support Groups (generic)
