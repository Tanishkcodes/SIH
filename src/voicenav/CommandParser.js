...
class CommandParser {
  ...
  _fastMatchIntent(input, context = {}) {
    ...
    // ──────────────────────────────────────────
    // 5B. VIEW DOCTOR PROFILE
    // ──────────────────────────────────────────
    if (
      /\b(?:doctor profile|dr profile|doctor ka profile|doctor ki profile|profile of doctor|physician profile|view doctor profile|show doctor profile|open doctor profile|doctor details|doctor bio|profile dikhao doctor)\b/i.test(raw) ||
      /^(doctor profile|dr profile|physician profile|doctor jankari|doctor parichay)$/i.test(raw) ||
      (/\bprofile\b/i.test(raw) && /\b(?:doctor|dr\.?|vaidya|physician|chikitsak|maruthuvar)\b/i.test(raw))
    ) {
      const doctorName = raw
        .replace(/\b(?:open|show|view|kholo|dikhao|doctor profile|dr profile|doctor ka profile|doctor ki profile|profile of doctor|profile|details|bio|about|of|ka|ki|ke|jankari|parichay|doctor|dr\.?|vaidya|physician|chikitsak|maruthuvar)\b/gi, ' ')
        .replace(/\s+/g, ' ')
        .trim();
      return { intent: 'view_doctor_profile', confidence: 0.98, value: doctorName || raw, target: doctorName || null };
    }
    ...
    // ──────────────────────────────────────────
    // 11. VIEW COMMUNITIES / PARTICULAR COMMUNITY
    // ──────────────────────────────────────────
    if (
      /\b(?:community|group|circle|samudaay|samaj|forum|network|sanctuary|mandal|samuh)\b/i.test(raw)
    ) {
      const extractedTopic = raw
        .replace(/\b(?:open|show|view|kholo|dikhao|jao|chalo|join|go to|communities|community|group|circle|samudaay|samaj|forum|network|sanctuary|mandal|samuh|the|a|an|in|me|mein|tab|particular)\b/gi, ' ')
        .replace(/\s+/g, ' ')
        .trim();
      if (extractedTopic.length >= 2) {
        return { intent: 'select_community', confidence: 0.98, value: extractedTopic, target: extractedTopic };
      }
    }
    ...
    if (
      ...
      /^(communities|community|samudaay|samaj|समुदाय|சமூகங்கள்|కమ్యూనిటీలు|সম্প্রদায়|સમુદાય|ಸಮುದಾಯಗಳು|കമ്മ്യൂണിറ്റികൾ)$/i.test(raw) ||
      /\b(?:open communities|open community|show communities|view community|communities|community)\b/i.test(raw)
    ) {
