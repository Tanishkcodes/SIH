import { validateIntent } from '../voicenav/ActionSnapshot.js';
...
class AICommandEngine {
  ...
  async parseIntent(transcript, availableCommands = {}, globalCommands = {}, ctx = {}) {
    const actions = Object.entries({ ...globalCommands, ...availableCommands }).map(([intent, description]) => ({
      intent, description: Array.isArray(description) ? description.join(' | ') : String(description),
    }));
    // No cached model decisions or browser provider keys: every turn uses live state.
    const result = await voiceAIService.understand({
